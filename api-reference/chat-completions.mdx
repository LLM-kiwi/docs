...
| :--- | :--- | :--- | :--- |
| `model` | string | Yes | Model ID: `default` (Free), or any Pro model (e.g. `gpt-oss-20b`). |
| `messages` | array | Yes | Conversation history as message objects. |
| `stream` | boolean | No | Enable streaming responses. Default: `false`. |
| `temperature` | number | No | Sampling temperature (0-2). Default: `1`. |
| `max_tokens` | integer | No | Maximum tokens to generate. |
| `response_format` | object | No | Set `{ "type": "json_object" }` for JSON mode. <sup>Pro</sup> |
| `tools` | array | No | Function definitions for tool calling. <sup>Pro</sup> |
| `tool_choice` | string/object | No | Control tool selection behavior. <sup>Pro</sup> |
